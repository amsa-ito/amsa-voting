 ...
 */
 ...
 function amsa_voting_display_result_card($vote_type_label, $key, $votes, $users_per_vote, $is_anonymous, $is_council_master, $is_institutional_weighted){
    ?>
    ...
    <div class="amsa-voting-card amsa-voting-card-<?php echo str_replace(' ', '-', $vote_type_label)?>">
        ...
        <h5 class="amsa-voting-card-title"><?php echo(($is_institutional_weighted && !$is_council_master) ? "Hidden":$vote_type_label.': '. $votes[$key]); ?></h5>
        </div>
                ...
                if($is_anonymous && !$is_council_master){
                    ...
                    foreach($users_per_vote[$key] as $user_id){
                        ...
                        if ($user) {
                            ...
                            echo('<li>'.htmlspecialchars($display_name) . $university_display . "</li>");
                        }
                    }
                }
            }
...
<div class="amsa-voting-result-row">
    <?php
    amsa_voting_display_result_card('Votes for', 'for', $votes, $users_per_vote, $is_anonymous, $is_council_master, $is_institutional_weighted);
    amsa_voting_display_result_card('Votes against', 'against', $votes, $users_per_vote, $is_anonymous, $is_council_master, $is_institutional_weighted); 
    amsa_voting_display_result_card('Abstentions', 'abstain', $votes, $users_per_vote, $is_anonymous, $is_council_master, $is_institutional_weighted); 
    ?>
</div>