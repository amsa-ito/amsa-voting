jQuery(document).ready(function($) {
	...
	$('[id^=amsa-voting-speaker-list-wrapper-]').on('click', '.speaker-removal-button', function(e) {
		...
		var data = {
			nonce: Theme_Variables.nonce,
			action: 'amsa_retract_nomination',
			block_id: block_id,
	...
	$('#real-time-update').change(function () {
		if ($(this).is(":checked")) {
			...
			var intervalId = setInterval(function () {
				$.ajax({
					...
					success: function (response) {
						$('#amsa-voting-speaker-list-wrapper-'+blockId).html(response.data['rendered_content']);
			...
			// Store intervalId in a data attribute for future reference
