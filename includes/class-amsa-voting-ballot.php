        ...
        if ($current_user_id>0){
            if(isset($_POST['candidate_preference']) && isset($_POST['post_id'])){
                    ...
                    $preferences = $_POST['candidate_preference'];
                                ...
                                // Filter out empty values before checking for duplicates
                    $filtered_preferences = array_filter($preferences, function($value) {
                        return !empty($value);
                    });

                    // Check for duplicate preference numbers
                    if (count($filtered_preferences) !== count(array_unique($filtered_preferences))) {
